...
| 유사도 | **코사인 유사도** (취향 centroid 대비 대표성 + 비슷한 채널 Top 3) | [app.py:292-347](app.py#L292-L347) | O(n²)·희소 벡터 | 카테고리 벡터 기반 채널 간 유사도 |
...
> 이전 버전 README에 적혀 있던 BFS, Z-score, K-means, 허프만, 블룸 필터, 가상 스크롤, Reingold-Tilford 등은 **실제 코드에 없습니다**. (코사인 유사도·카테고리 분류·안정 정렬 2차키는 이후 추가되어 위 표에 반영됨.)
