...
> Google OAuth로 로그인 → 구독 채널을 **관심도 순**으로 분석 → **AI 분석 도구**와 **취향 그래프**까지 한 화면에서.

![분석 결과 — 관심도 순위 + AI 분석 도구](static/images/우하단.png)
...
| 1·2단계 — 클라이언트 ID 설정 & Google 로그인 | 3단계 — (선택) Takeout 업로드 |
|:---:|:---:|
| ![로그인 화면](static/images/기본화면.png) | ![로그인 후 Takeout 업로드](static/images/로그인%20후.png) |
...
| 시청 취향 페르소나 (`/api/persona`) | 구독 정리 코치 (`/api/cleanup`) |
|:---:|:---:|
| ![시청 취향 페르소나](static/images/페르소나.png) | ![구독 정리 코치](static/images/구독%20정리%20코치.png) |
...
![새 채널 추천 결과](static/images/채널%20추천.png)

또한 구독 목록뿐 아니라 **최근 시청 기록**(Takeout `watch-history`)을 함께 반영해, *취향 기반 새 채널*과 *최근 봤지만 구독 안 한 채널*을 함께 추천한다. 카테고리 분류로 관심사를 도출해 **요리·경제·게임 등 카테고리마다 최소 1개씩** 다양하게 추천한다.

| ✨ 취향 기반 새 채널 추천 | 📺 최근 봤지만 구독 안 한 채널 |
|:---:|:---:|
| ![취향 기반 추천](static/images/취향%20기반%20추천.png) | ![최근 본 비구독 채널](static/images/최근%20본.png) |
...
**취향 커뮤니티(Union-Find 연결요소) · 취향 지도(Kruskal MST)** — 카테고리 코사인 유사도가 임계값 이상인 채널쌍을 엣지로 보고, 유사 채널을 커뮤니티로 묶어(Union-Find) 대표 카테고리로 라벨링하고, 최대 신장 포레스트(Kruskal MST)로 취향을 잇는 백본을 force-directed SVG로 시각화한다.

![취향 커뮤니티 + force-directed 취향 지도](static/images/그래프%20추가.png)
